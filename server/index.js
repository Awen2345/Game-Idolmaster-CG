...
// Serve Idol Card Images
app.use('/assets/idols', express.static(path.join(__dirname, '../public/idols')));
// Serve Character Sprites: Expects folder structure public/sprites/{idolName}/{expression}.png
...
app.post('/api/auth/register', (req, res) => {
    ...
    const now = Date.now();
    // Simplified registration without hash for demo
...
app.get('/api/user/:id/idols', (req, res) => {
  const userId = req.params.id;
  const sql = `
    SELECT ui.id, ui.level, ui.isLocked, it.* 
    FROM user_idols ui 
    ...
    db.get("SELECT * FROM events WHERE start_time <= ? AND end_time >= ? LIMIT 1", [now, now], (err, event) => {
        ...
        db.get("SELECT points FROM event_points WHERE user_id = ? AND event_id = ?", [userId, event.id], (err, pointRow) => {
            ...
            db.all("SELECT * FROM event_rewards_def WHERE event_id = ? ORDER BY point_threshold ASC", [event.id], (err, rewards) => {
                ...
                // Get Top 5 Ranking
                db.all(`SELECT u.name, ep.points FROM event_points ep 
                        JOIN users u ON ep.user_id = u.id 
                        WHERE ep.event_id = ? 
                        ORDER BY ep.points DESC LIMIT 5`, [event.id], (err, ranking) => {
...
app.post('/api/event/work', (req, res) => {
    ...
    db.get("SELECT stamina, maxStamina, exp, level, maxExp FROM users WHERE id = ?", [userId], (err, user) => {
        ...
        db.run("UPDATE users SET stamina = ?, exp = ?, level = ?, maxStamina = ? WHERE id = ?", 
               [newStamina, newExp, newLevel, newMaxStamina, userId]);
        ...
        db.get("SELECT points FROM event_points WHERE user_id = ? AND event_id = ?", [userId, eventId], (err, row) => {
            if (row) {
                db.run("UPDATE event_points SET points = points + ? WHERE user_id = ? AND event_id = ?", [pointsGained, userId, eventId]);
            } else {
                db.run("INSERT INTO event_points VALUES (?, ?, ?)", [userId, eventId, pointsGained]);
            }
...
app.get('/api/commu/chapters', (req, res) => {
    const { type } = req.query; // STORY, EVENT, IDOL, EXTRA
...
// --- FANMADE STORY ROUTES ---

app.get('/api/fan/chapters', (req, res) => {
    const sql = `
        SELECT fc.*, u.username as authorName 
        FROM fan_chapters fc 
...
app.get('/api/fan/dialogs/:chapterId', (req, res) => {
    db.all("SELECT * FROM fan_dialogs WHERE chapter_id = ? ORDER BY sort_order ASC", [req.params.chapterId], (err, rows) => {
        // Map database column custom_sprite_url to object property customSpriteUrl
        const mappedRows = rows.map(r => ({
            ...r,
            customSpriteUrl: r.custom_sprite_url
        }));
        res.json(mappedRows);
    });
});
...
app.post('/api/fan/create', (req, res) => {
    ...
    if (!title || !dialogs || dialogs.length === 0) {
        return res.status(400).json({ error: "Invalid story data" });
    }

    db.run("INSERT INTO fan_chapters (user_id, title, created_at) VALUES (?, ?, ?)", 
...
app.post('/api/fan/sprite', (req, res) => {
    const { userId, name, image } = req.body; // image is base64 string
...
// --- EXISTING ROUTES (Gacha, Item, Shop, Train, Retire) ---
  ...
  db.get("SELECT starJewels FROM users WHERE id = ?", [userId], (err, row) => {
    ...
    db.all("SELECT * FROM idol_templates", (err, templates) => {
      ...
      for(let i=0; i<count; i++) {
        ...
        if (rand <= 5) { pool = templates.filter(t => t.rarity === 'SSR'); selectedRarity = 'SSR'; }
        else if (rand <= 20) { pool = templates.filter(t => t.rarity === 'SR'); selectedRarity = 'SR'; }
        else if (rand <= 50) { pool = templates.filter(t => t.rarity === 'R'); selectedRarity = 'R'; }

        if (count === 10 && i === 9 && (selectedRarity === 'N' || selectedRarity === 'R')) {
          pool = templates.filter(t => t.rarity === 'SR' || t.rarity === 'SSR');
        }
...
app.post('/api/item/use', (req, res) => {
  ...
  if (item === 'staminaDrink') {
    db.get("SELECT count FROM user_items WHERE user_id = ? AND item_name = 'staminaDrink'", [userId], (err, row) => {
      if (!row || row.count <= 0) return res.status(400).json({ error: "No items" });
      
      db.run("UPDATE user_items SET count = count - 1 WHERE user_id = ? AND item_name = 'staminaDrink'", [userId]);
      db.get("SELECT maxStamina, stamina FROM users WHERE id = ?", [userId], (err, u) => {
        const newStamina = Math.min(u.maxStamina, u.stamina + 20);
        db.run("UPDATE users SET stamina = ? WHERE id = ?", [newStamina, userId]);
        res.json({ success: true, newStamina, newCount: row.count - 1 });
      });
    });
  } else {
    res.status(400).json({ error: "Item not usable directly" });
  }
});
  ...
  db.get("SELECT starJewels FROM users WHERE id = ?", [userId], (err, row) => {
    ...
    db.get("SELECT count FROM user_items WHERE user_id = ? AND item_name = ?", [userId, item], (err, iRow) => {
        if (iRow) {
            ...
            db.run("INSERT INTO user_items VALUES (?, ?, 1)", [userId, item]);
        }
...
app.post('/api/idol/train', (req, res) => {
  ...
  db.get("SELECT count FROM user_items WHERE user_id = ? AND item_name = 'trainerTicket'", [userId], (err, iRow) => {
    if (!iRow || iRow.count <= 0) return res.status(400).json({ error: "No tickets" });
